...
document.getElementById("editBtn").addEventListener("click", async function() {
    ...
    if (data.role !== "admin" && data.role !== "customer") {
        document.getElementById("roleError").textContent = "Please select a valid role.";
        hasError = true;
    }
    if (hasError) return;
        ...
        alert("Updated successfully!");
            ...
            window.location.href = "http://localhost/NTI/final_project/Admin/get_all_users.php";
        
    function clearErrors() {
        ["fName", "lName", "email", "password" , "role"].forEach(id => {
